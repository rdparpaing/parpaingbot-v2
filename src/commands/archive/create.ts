...
async function repostAttachment(
  ...
  const backend_channel =
    interaction.client.channels.cache.get("931558530522177566");
...
export default async function (interaction: ChatInputCommandInteraction) {
  ...
  const fichier = await repostAttachment(
    interaction.options.getAttachment("fichier", false),
    interaction
  );

  if (!comment && !fichier) {
    await interaction.reply(
      "❌ Veuillez fournir au minimum un fichier ou un commentaire."
    );
    return;
  }
